...
class GuestStoreMapPage extends StatefulWidget {
  ...
  const GuestStoreMapPage(
      {super.key, required this.shops, this.initialPosition});
...
class _GuestStoreMapPageState extends State<GuestStoreMapPage>
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.shops.isNotEmpty && widget.shops[0].latitude != null) {
      startPoint =
          LatLng(widget.shops[0].latitude!, widget.shops[0].longitude!);
    }
                      ...
                      return AnimatedScale(
                        ...
                        child: Opacity(
                          ...
                          child: _buildShopCard(
                              widget.shops[index], _currentIndex == index),
                        ),
                      );
                    },
                  ),
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      Navigator.pushReplacement(
                        ...
                        MaterialPageRoute(
                            builder: (context) =>
                                GuestMenuScreen(shopId: currentShop.id)),
                      );
                    },
...
Widget _buildCustomPin(bool isSelected) {
  final double size = isSelected ? 40 : 30;

  return SizedBox(
  width: isSelected ? 40 : 30,
  height: isSelected ? 70 : 35, // taller than wide
  child: Icon(
              ...
              child: (shop.imageUrl != null && shop.imageUrl!.isNotEmpty)
                  ? Image.network(shop.imageUrl!,
                      ...
                      errorBuilder: (_, __, ___) =>
                          Icon(Icons.store, color: _primaryGreen))
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(shop.location ?? "Nearby",
                    style: TextStyle(fontSize: 12, color: Colors.grey[600]),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis),
                const SizedBox(height: 8),
                Text(
                    "${shop.distanceInKm?.toStringAsFixed(1) ?? '0.0'} km away",
                    style: TextStyle(
                        fontSize: 12,
